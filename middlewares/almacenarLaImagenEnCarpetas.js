  ...
  destination: async (req, file, cb) => {
    ...
    if (fs.existsSync(path.join(__dirname, `/../uploads/${categoria}`))) {
      if (fs.existsSync(path.join(__dirname, `/../uploads/${categoria}/${req.body.nombre}`)) === false) {
        ...
        await fs.mkdir(path.join(__dirname, `/../uploads/${categoria}/${req.body.nombre}`), err => console.log(err))
      ...
      await fs.mkdir(path.join(__dirname, `/../uploads/${categoria}`), err => console.log(err))
      await fs.mkdir(path.join(__dirname, `/../uploads/${categoria}/${req.body.nombre}`), err => console.log(err))
